...
class AnalyzeTests(unittest.TestCase):
    ...
    def test_get_low_and_high_prices_return_correct_dictionary(self):
        ...
        month = price_stats[Period.MONTH].round()
        self.assertEqual(month, self._get_expected_month())

        quarter = price_stats[Period.QUARTER].round()
        self.assertEqual(quarter, self._get_expected_quarter())

        half = price_stats[Period.HALF].round()
        self.assertEqual(half, self._get_expected_half())

        year = price_stats[Period.YEAR].round()
    ...
    def test_get_volatility_return_correct_dictionary(self):
        ...
        prices = pd.read_csv('csv_files/AMZN_from_stockbot.csv')
        # act
        volatility = analyze.get_volatility(prices)
        # assert
        self.assertEqual(len(volatility), 4)
        self.assertEqual(volatility[Period.MONTH], 0.1536)
        self.assertEqual(volatility[Period.QUARTER], 0.3148)
        self.assertEqual(volatility[Period.HALF], 0.3995)
        self.assertEqual(volatility[Period.YEAR], 0.4350)
